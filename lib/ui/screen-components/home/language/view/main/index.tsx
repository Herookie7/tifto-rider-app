...
export default function LanguageMain() {
  ...
  const handleLanguageSelection = async (selectedLanguage: string) => {
    ...
    await AsyncStorage.setItem("tifto-language", selectedLanguage);
  };
  const handleSetCurrentLanguage = async () => {
    try {
      const lng = await AsyncStorage.getItem("tifto-language");
      if (lng) {
        setIsSelected(lng);
      }
    } catch (error) {
  ...
  const handleSubmission = async () => {
    try {
      ...
      await AsyncStorage.setItem("tifto-language", isSelected);
